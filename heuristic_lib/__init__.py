...
import xlsxwriter
import numpy as np
from heuristic_lib import algorithms, benchmarks

__all__ = ['algorithms', 'benchmarks']
__project__ = 'heuristic_lib'
__version__ = '1.0.0'

VERSION = "{0} v{1}".format(__project__, __version__)

logging.basicConfig()
logger = logging.getLogger('heuristic_lib')
...
class Runner:
    ...
    def __algorithmFactory(self, name, benchmark):
        ...
        elif name == 'RedFoxOptimizationAlgorithm':
            algorithm = algorithms.custom.RedFoxOptimizationAlgorithm(
                self.D, self.NP, self.nFES, bench)
        else:
