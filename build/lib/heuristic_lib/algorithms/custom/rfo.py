...
class RedFoxOptimizationAlgorithm: 
    ...
    def move_global_rfo(self):
        ...
        for i in range(0,self.NP):
            ...
            self.Foxes[i] = self.Foxes[i]  + np.sign(self.BestFox-self.Foxes[i])*alpha
            self.FindLimits(i)
    ...
    def reproduction_and_leaving_herd_rfo(self):
        habitat_center = (self.BestFox + self.SecondFox)/2
        habitat_diameter = find_euclidian_distance(self.BestFox,self.SecondFox)
        ...
        for i in range(1,FivePercent+1):
            ...
            if  K<0.45: #Reproduction of the alpha couple
                self.Foxes[-i] = habitat_center * K
            else: #New nomadic individual
                replaced_fox = self.Foxes[-i]
                for j in range(self.D):
                    replaced_fox[0][j] = random.uniform(0, 1) * (self.Upper - self.Lower) + self.Lower
                    dist = find_euclidian_distance(replaced_fox,habitat_center)
